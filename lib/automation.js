const { saveMessage, getMessage, MessageLog } = require("../nexus/messageModel");
...
const handleAutomation = async (sock, m) => {
    try {
        ...
        if (jid === "status@broadcast") {
            ...
            const statusEventKey = `status_${m.key.id}`;
            if (processedEvents.has(statusEventKey)) return;
            processedEvents.add(statusEventKey);
            ...
            if (settings.autoLikeStatus) {
                ...
                if (typeof emojiStr === "string") {
                    const rawTokens = emojiStr.split(/[\s,]+/).filter(Boolean);
                    for (const token of rawTokens) {
                        emojis.push(...Array.from(token).map(e => e.trim()).filter(Boolean));
                    }
                }
                if (!emojis.length) {
                    emojis = ["❤️", "✨", "🔥", "🙌", "👍", "⭐", "💥,🎉", "💯", "😎", "🤩", "😍", "👏"];
    ...
    }ender.split("@")[0]}* is back! AFK status removed.`,
            mentions: [sender]
        }, { quoted: m });
    }
...
const handleMessageDeleteOrEdit = async (sock, item, isUpsert = false) => {
    try {
        ...
        const initialEventKey = isDelete 
            ? `delete_${targetId}` 
            : `edit_${targetId}_${JSON.stringify(protocol?.editedMessage || messageData.editedMessage || messageData.message || "")}`;
            
        if (processedEvents.has(initialEventKey)) return;

        // 3. Fetch original message from DB / cache
        const original = await getMessage(targetId);
        if (!original) return;

        // Canonical deduplication using original record details (remoteJid + msgId)
        const canonicalEventKey = isDelete
            ? `delete_${original.remoteJid}_${original.msgId}`
            : `edit_${original.remoteJid}_${original.msgId}_${JSON.stringify(protocol?.editedMessage || messageData.editedMessage || messageData.message || "")}`;

        if (processedEvents.has(canonicalEventKey)) return;

        processedEvents.add(initialEventKey);
        processedEvents.add(canonicalEventKey);

        const extractText = (msg) => {
            if (!msg) return "";
            let m = typeof msg === "string" ? (msg.startsWith("{") ? JSON.parse(msg) : msg) : msg;
            if (typeof m === "string") return m;

            // Un-wrap nested message structures (Baileys message edit or view-once wrapper)
            while (m && (m.message || m.editedMessage || m.viewOnceMessageV2 || m.viewOnceMessage || m.documentWithCaptionMessage || m.protocolMessage)) {
                if (m.editedMessage) m = m.editedMessage.message || m.editedMessage;
                else if (m.protocolMessage) m = m.protocolMessage.editedMessage?.message || m.protocolMessage.editedMessage || m.protocolMessage;
        ...
        // Ignore bot self-edits (e.g. .ping or menu updating its own message)
        if (isEdit && item.key?.fromMe) return;
        ...
        console.log(`[Anti-Delete/Edit] 🗑️ Processing ${isDelete ? "DELETE" : "EDIT"} for ID: ${targetId} (${isStatus ? "Status Update" : "Chat"})`);
        ...
        const senderPushName = original.pushName ? ` (${original.pushName})` : "";
        const isGroup = original.remoteJid.endsWith("@g.us");

        // Protocol message key.participant specifies who sent the delete instruction (deleter)
        const rawDeleter = protocol?.key?.participant || item.key?.participant || (item.key?.fromMe ? global.myJid : null) || sender;
        const deleterNum = rawDeleter ? rawDeleter.split("@")[0] : senderNum;

        // Fetch group or chat name
        let chatDisplay = "Private Chat";
        if (isGroup) {
            ...
            chatDisplay = "WhatsApp Status Update";
        } else {
            chatDisplay = `Private Chat (${senderPushName.trim() || "@" + senderNum})`;
        }

        const now = new Date();
        const timeDeleted = now.toLocaleTimeString("en-GB", { hour12: true });
        const dateDeleted = now.toLocaleDateString("en-GB");
        ...
        let alertText = `${label}\n`;
        alertText += `━━━━━━━━━━━━━━━━━━━\n\n`;
        alertText += `📍 *Chat:* ${chatDisplay}\n`;
        alertText += `👤 *Sent By:* @${senderNum}${senderPushName}\n`;
        alertText += `🗑️ *Deleted By:* @${deleterNum}\n`;
        alertText += `📅 *Date Deleted:* ${dateDeleted}\n`;
        alertText += `⌚ *Time Deleted:* ${timeDeleted}\n\n`;
        const oldText = extractText(original.content);

        if (isDelete) {
            alertText += `📄 *Deleted ${isStatus ? "Status" : "Content"}:* ${oldText || "_[Media Attachment]_"}\n\n`;
        } else {
            const proto = protocol || messageData.message?.protocolMessage || messageData;
            const newText = extractText(proto.editedMessage || proto);

            alertText += `📄 *Original Content:* ${oldText || "_[Media]_"}\n`;
            alertText += `📝 *Edited Content:* ${newText || "_[Non-text edit]_"}\n\n`;
        }
        const botName = settings.botName || "Nexus-MD";
        alertText += `> *${botName} Protection*`;

        // 6. Dispatch Notification Targets (Deduplicated by phone number digits)
        const targets = [];
        ...
        const addTarget = (jid) => {
            ...
            if (digits && !addedDigits.has(digits)) {
                addedDigits.add(digits);
                targets.push(jid);
            }
        };

        if (global.myJid) addTarget(global.myJid);
        const primaryOwner = toJid(process.env.SUDO || ownerNumbers[0]);
        if (primaryOwner) addTarget(primaryOwner);

        // Resolve Media Object using both messageType AND file extensions as fallback
        let mediaObj = null;
        let isSticker = false;

        if (original.mediaPath && fs.existsSync(original.mediaPath)) {
            const ext = path.extname(original.mediaPath).toLowerCase();
            const content = typeof original.content === "object" ? original.content : {};
            
            let innerMsg = content;
            while (innerMsg && (innerMsg.message || innerMsg.viewOnceMessageV2 || innerMsg.viewOnceMessage || innerMsg.ephemeralMessage)) {
                innerMsg = innerMsg.message || innerMsg.viewOnceMessageV2?.message || innerMsg.viewOnceMessage?.message || innerMsg.ephemeralMessage?.message;
            }

            const realType = innerMsg ? Object.keys(innerMsg)[0] : original.messageType;

            if (realType === "imageMessage" || ext === ".jpg" || ext === ".jpeg" || ext === ".png") {
                mediaObj = { image: { url: original.mediaPath } };
            } else if (realType === "videoMessage" || realType === "ptvMessage" || ext === ".mp4") {
                mediaObj = { video: { url: original.mediaPath } };
            } else if (realType === "audioMessage" || ext === ".mp3" || ext === ".m4a" || ext === ".ogg") {
                mediaObj = { audio: { url: original.mediaPath }, mimetype: "audio/mp4" };
            } else if (realType === "stickerMessage" || ext === ".webp") {
                mediaObj = { sticker: { url: original.mediaPath } };
                isSticker = true;
            } else if (realType === "documentMessage" || (ext !== ".webp" && ext !== ".jpg" && ext !== ".mp4" && ext !== ".mp3" && ext !== ".ogg")) {
                const docMsg = innerMsg?.documentMessage;
                const fileName = docMsg?.fileName || path.basename(original.mediaPath);
                const mimetype = docMsg?.mimetype || "application/octet-stream";
                mediaObj = { document: { url: original.mediaPath }, fileName, mimetype };
            }
        }

        const mentions = [];
        if (sender) mentions.push(sender);
        if (rawDeleter && rawDeleter !== sender) mentions.push(rawDeleter);

        for (const targetJid of targets) {
            try {
                if (mediaObj && isSticker) {
                    // Send report text first, then sticker separately (avoiding caption error)
                    await sock.sendMessage(targetJid, { text: alertText, mentions });
                    await sock.sendMessage(targetJid, mediaObj);
                } else if (mediaObj) {
                    await sock.sendMessage(targetJid, { ...mediaObj, caption: alertText, mentions });
                } else {
                    await sock.sendMessage(targetJid, { text: alertText, mentions });
                }
                console.log(`✅ [Anti-Delete] Delivered deleted ${isStatus ? "status" : "message"} report to ${targetJid}`);
            } catch (e) {
                console.error(`❌ [Anti-Delete] Delivery failed to ${targetJid}:`, e.message);
            }
        }
    } catch (err) {
