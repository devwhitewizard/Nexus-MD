...
// Import root wrapper settings if present
let rootWrapper = {};
try {
    rootWrapper = require("../settings");
} catch (e) { }

const defaultSettings = { 
    publicMode: parseBoolEnv(process.env.MODE_PUBLIC || process.env.PUBLIC_MODE || process.env.MODE || global.mode, true),
    ...
    ownerNumber: parseStrEnv(process.env.OWNER_NUMBER || (global.owner ? global.owner.toString() : ''), ''),
    lockedCommands: parseStrEnv(process.env.LOCKED_COMMANDS, ''),
    botName: parseStrEnv(process.env.BOT_NAME || global.botname, 'Nexus-MD'),
    device: parseStrEnv(process.env.DEVICE, 'Android'),
    prefix: parseStrEnv(process.env.PREFIX || global.prefix, '.'),
...
const updateSettings = async (updates) => {
    ...
    if (settingsCache) {
        ...
        // Force immediate sync to jsonStore as well to guarantee persistence
        const jsonStore = require("../nexus/jsonStore");
        jsonStore.set("settings_1", settingsCache.dataValues || settingsCache, true);
    }
    return settingsCache;
};
